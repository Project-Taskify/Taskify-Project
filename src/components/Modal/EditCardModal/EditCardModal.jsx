...
const EditCardModal = ({ isOpen, cardInfo, columnId, onClose }) => {
  ...
  const { mutateAsync: updateCard } = useMutation({
    mutationFn: async (cardInfo) => {
      const { data } = await instance.put(`/cards/${cardInfo.cardId}`, cardInfo);
      return data;
    },
  });
  ...
  useEffect(() => {
    const { title, assignee, description, dueDate, tags, imageUrl } = cardInfo;

    setValue('assignee', assignee.id);
  ...
  return (
    <BaseModal isOpen={isOpen}>
      <S.Container>
        ...
        <S.FieldWrapper>
          <S.SelectWrapper style={{ display: 'flex' }}>
            <div>
              <label>상태</label>
        ...
        <S.FieldWrapper>
          ...
          <input type="datetime-local" id="endDate" {...register('dueDate')} />
        </S.FieldWrapper>
        <S.FieldWrapper>
